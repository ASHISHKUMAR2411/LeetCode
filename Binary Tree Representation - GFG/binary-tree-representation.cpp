...
public:
    ...
    void create_tree(node* root0, vector<int> &v){
        ...
        {
            // the root node that has been stored in the queue should be used to construct the tree
            auto temp=q.front();
            //now remove that from the queue
            q.pop();
            //now our temp has root node
            //create a left for root node
            temp->left=newNode(v[i]);
            q.push(temp->left);
            //now right node for root node
